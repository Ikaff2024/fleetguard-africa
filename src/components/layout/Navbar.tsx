...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  /**
   * Le sélecteur d'organisation n'a de sens qu'en démonstration.
   *
   * Une fois authentifié, le tenant vient du jeton signé : changer
   * d'organisation depuis le navigateur n'a aucun effet. Le menu listait
   * pourtant toutes les organisations du jeu de démonstration — un exploitant
   * de TransAfrik y lisait le nom de Sahel Express, et pouvait croire qu'il
   * lui suffisait de cliquer pour voir sa flotte. L'isolation n'était pas en
   * cause ; l'écran, si.
   */
  const canSwitchOrganization = status === 'demonstration';
  ...
  return (
    <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-100 px-6 py-3 flex flex-wrap items-center justify-between gap-4 sticky top-0 z-50 shadow-xs transition-colors">
      ...
      <div className="flex items-center flex-wrap gap-3 text-xs">
        ...
        {/* Organisation courante ; sélecteur en démonstration seulement. */}
        <div className={canSwitchOrganization ? 'relative group' : 'relative'}>
          <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200/80 dark:hover:bg-slate-700/80 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-lg text-slate-800 dark:text-slate-200 cursor-pointer transition font-medium">
            ...
            {canSwitchOrganization && <ChevronDown className="w-3.5 h-3.5 text-slate-400" />}
          </div>
